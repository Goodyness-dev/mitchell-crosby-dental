      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-3 sm:mt-4 text-base sm:text-xl leading-relaxed">
            From single-visit CEREC® same-day crowns to dental implants and gentle family checkups — select a treatment to get started.
          </p>
        </div>
            ...
            >
              ...
              <div className="relative h-80 sm:h-96 lg:h-[420px] w-full">
                <img
                  ...
                  alt={`${service.title || service.name} - ${BUSINESS_INFO.name} Casa Grande, AZ`}
                  loading="lazy"
                  decoding="async"
                  width="360"
                  height="420"
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6">
                ...
                <p className="text-white/90 text-sm sm:text-base leading-relaxed mb-4 line-clamp-2">
                  {(service.description || service.shortDesc || '').split('.')[0]}.
                </p>
                ...
                <div className="flex items-center gap-2.5 mb-4">
                  ...
                  {service.popular && (
                    <span className="px-3 py-1 rounded-full bg-shop-red text-white text-xs sm:text-sm font-semibold">
                      Popular
                    </span>
                  )}
                </div>
                ...
                {/* CTA Button */}
                <button
                  className="w-full py-3 rounded-xl bg-white/25 backdrop-blur-md hover:bg-shop-red text-white font-bold text-sm sm:text-base transition-all border border-white/25 hover:border-shop-red flex items-center justify-center space-x-2 cursor-pointer"
                  aria-label={`Book ${service.title || service.name} consultation`}
                >
        ...
        <div className="text-center">
          <button
            ...
            className="inline-flex items-center space-x-2.5 px-8 py-4 rounded-xl bg-gray-900 dark:bg-[#0c0c0c] hover:bg-gray-800 dark:hover:bg-[#161616] text-white font-bold text-base sm:text-lg transition-all shadow-md active:scale-95 border border-transparent dark:border-neutral-800 cursor-pointer"
            aria-label={`View complete catalog of ${SERVICES.length} dental services`}
          >
            <LayoutGrid className="w-5 h-5" />
            <span>View All {SERVICES.length} Dental Services</span>
