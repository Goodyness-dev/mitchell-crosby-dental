...
export default function QuoteDetailModal({ quote, onClose, onUpdate }) {
  ...
  const [message, setMessage] = useState(
    quote?.adminMessage || 
    `Hi ${quote?.name || 'Patient'}, thanks for reaching out to ${BUSINESS_INFO.name}! I reviewed your consultation request for ${quote?.detailedService || quote?.serviceCategory || 'dental care'}. Give our office a call at ${BUSINESS_INFO.phone} or reply here to confirm your appointment time.`
  );
              ...
              <form onSubmit={handleSendQuote} className="space-y-5 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-slate-400 mt-1 block">Tells patient when appointment or restoration will be ready.</span>
                  </div>
                </div>
