...
export default function QuoteDetailModal({ quote, onClose, onUpdate }) {
  ...
  const [warranty, setWarranty] = useState(quote?.warrantyNote || 'Mitchell & Crosby Clinical Quality Guarantee');
  const [message, setMessage] = useState(
    quote?.adminMessage || 
    `Hi ${quote?.name || 'Patient'}, thank you for reaching out to ${BUSINESS_INFO.name}! I reviewed your consultation request for ${quote?.detailedService || quote?.serviceCategory || 'dental care'}. We would love to welcome you into our office. Please give us a call at ${BUSINESS_INFO.phone} or reply here to finalize your visit.`
  );
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Are you sure you want to delete Patient Record #${quote.id}?`)) return;
    setIsDeleting(true);
    try {
      ...
      >
        ...
        <div className="px-5 sm:px-6 py-3 bg-slate-50 border-b border-slate-200/80 flex flex-wrap items-center gap-3 sm:gap-4 text-xs text-slate-600">
          ...
          <div className="text-slate-500 font-mono">
            <span>Casa Grande, AZ Clinic</span>
          </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-6 bg-slate-50/40">
          {activeTab === 'quote_studio' ? (
              ...
              {sendSuccess && (
                <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm flex items-center space-x-3 shadow-xs">
                  ...
                  <div>
                    ...
                    <span>An official branded summary email with your consultation estimate has been delivered to the patient.</span>
                  </div>
                </div>
              )}
              ...
              <form onSubmit={handleSendQuote} className="space-y-5 bg-white p-5 sm:p-6 rounded-2xl border border-slate-200/80 shadow-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    />
                    <span className="text-[11px] text-slate-400 mt-1 block">Provides the patient with scheduling availability.</span>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2 font-mono">
                    Care & Workmanship Assurance
                  </label>
                  <input
                    type="text"
                    value={warranty}
                    onChange={(e) => setWarranty(e.target.value)}
                    placeholder="e.g. Mitchell & Crosby Clinical Quality Guarantee"
                    className="w-full bg-slate-50 border border-slate-200 focus:border-shop-red focus:bg-white rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-400 outline-none transition"
                  />
                </div>

                {/* Personal Message / Note to Patient */}
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2 font-mono">
                    Direct Clinical Note to Patient
                  </label>
                  <textarea
                    rows={4}
                    value={message}
                    onChange={(e) => setMessage(e.target.value)}
                    placeholder="Write a custom explanation, pre-appointment preparation, or insurance notes..."
                  ...
                  <span className="text-[11px] text-slate-400 mt-1 block">
                    This note is prominently highlighted in the patient's confirmation email.
                  </span>
                </div>
                ...
                <div className="pt-2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  ...
                  >
                    {isSendingQuote ? (
                      ...
                      <span>Dispatch Estimate to Patient →</span>
                    )}
                  </button>
                </div>
              </form>
            ...
            /* TAB 2: Full Patient Intake Details */
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl p-5 space-y-3 shadow-xs">
                  ...
                  <div className="flex justify-between text-sm border-b border-slate-100 pb-2">
                    ...
                    <span className="text-slate-900 font-bold">{quote.detailedService || quote.serviceCategory || 'Dental Consultation'}</span>
                  </div>
                  <div className="flex justify-between text-sm border-b border-slate-100 pb-2">
                    <span className="text-slate-500">Category:</span>
                    <span className="text-slate-900 font-bold">{quote.serviceCategory || 'General / Cosmetic Dentistry'}</span>
                  </div>
                  <div className="flex justify-between text-sm border-b border-slate-100 pb-2">
                    <span className="text-slate-500">Practice Location:</span>
                    <span className="text-slate-900 font-medium">Casa Grande, AZ</span>
                  </div>
              ...
              {(quote.details || quote.additionalNotes) && (
                <div className="bg-white border border-slate-200/80 rounded-2xl p-5 space-y-2 shadow-xs">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block font-mono">// PATIENT COMMENTS & SYMPTOMS</span>
              ...
              {quote.customIssue && quote.customIssue !== 'N/A' && (
                <div className="bg-red-50/50 border border-red-200 rounded-2xl p-5 space-y-2">
                  <span className="text-xs font-bold text-shop-red uppercase tracking-wider block font-mono">// SPECIFIC CLINICAL CONCERN</span>
              ...
              <div className="pt-4 border-t border-slate-200 flex justify-end">
                ...
                >
                  <span>[DELETE]</span>
                  <span>{isDeleting ? 'Removing...' : 'Delete Patient Record'}</span>
                </button>
              </div>
            </div>
          )}
